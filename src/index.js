...
async function batBuocXemRnD(req, env) {
  const { phien, loi: l } = await batBuocDangNhap(req, env);
  if (l) return { loi: l };
  if (!duocXemTab(phien.vai_tro, 'kinhdoanh')) return { loi: loi('Bạn không có quyền xem R&D', 403) };
  return { phien };
}
...
async function kdTachDongHang(req, env) {
  const { phien, loi: l } = await batBuocDangNhap(req, env);
  if (l) return l;
  if (!laAdmin(phien.vai_tro)) return loi('Chỉ Admin mới chạy được việc bóc dữ liệu này', 403);
  if (!(await donHangItem.coBangDong(env))) {
