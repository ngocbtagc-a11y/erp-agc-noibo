...
export async function danhSachDuAn(env, phien) {
  ...
  return json({
    ...
    giai_doan: GIAI_DOAN,
    quyen: {
      sua: duocSuaSanPham(phien.vai_tro),
      duyet: duocKhoaSanPham(phien.vai_tro)
    },
...
export async function taoDuAn(env, phien, body) {
  if (!duocSuaSanPham(phien.vai_tro)) return loi('Bạn không có quyền tạo dự án R&D', 403);
...
export async function suaDuAn(env, phien, body) {
  if (!duocSuaSanPham(phien.vai_tro)) return loi('Bạn không có quyền sửa dự án R&D', 403);
...
export async function capNhatBuoc(env, phien, body) {
  if (!duocSuaSanPham(phien.vai_tro)) return loi('Bạn không có quyền cập nhật bước R&D', 403);
...
export async function chuyenGiaiDoan(env, phien, body) {
  if (!duocSuaSanPham(phien.vai_tro)) return loi('Bạn không có quyền chuyển giai đoạn', 403);
  ...
  if (du_an.giai_doan === GIAI_DOAN_CAN_DUYET && !duocKhoaSanPham(phien.vai_tro)) {
  ...
  if (conThieu.length) {
    ...
    if (!duocKhoaSanPham(phien.vai_tro)) return loi('Chỉ Kinh doanh/Admin mới được bỏ qua việc bắt buộc', 403);
...
export async function doiTrangThai(env, phien, body) {
  if (!duocSuaSanPham(phien.vai_tro)) return loi('Bạn không có quyền đổi trạng thái dự án', 403);
  ...
  if (trangThai === 'huy' && !duocKhoaSanPham(phien.vai_tro)) {
...
export async function ganSanPham(env, phien, body) {
  if (!duocSuaSanPham(phien.vai_tro)) return loi('Bạn không có quyền gắn mã hàng', 403);
