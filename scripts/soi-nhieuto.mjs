...
createServer((req, res) => {
  ...
  if (u.pathname === '/' || u.pathname === '/index.html') {
    res.writeHead(200, { 'Content-Type': KIEU['.html'] });
    return res.end(readFileSync(path.join(THU_MUC, 'soi-nhieuto.html'), 'utf8'));
  }
  ...
  /* Ép lại khung nhìn KHÔNG dùng `mobile:true`: `moChrome` đặt `mobile` theo
     bề ngang, và `mobile:true` thiếu `screen*` thì Chrome co giãn khung —
     xin 375×812 mà `innerWidth` báo 981. Phép đo CHIỀU CAO MÀN chỉ có nghĩa
     khi khung nhìn đúng bằng số đã xin, nên in ra để tự kiểm. */
  await cr.goi('Emulation.setDeviceMetricsOverride',
    { width: 375, height: 812, deviceScaleFactor: 1, mobile: false,
      screenWidth: 375, screenHeight: 812 }, cr.sessionId);
  await cr.doi(600);
  console.log('[tự kiểm khung nhìn] ' +
    await cr.chay('window.innerWidth + "×" + window.innerHeight'));
  let chu = '';
