...
| Giao diện — 3 cặp màu dưới ngưỡng WCAG | Claude (phiên Sếp Ngọc 11/09) | `fix/tuong-phan-mau` | `do-tuong-phan-mau` báo 3 TRƯỢT trên main: nút cam màn quét tài liệu, nút × đóng khung xem tài liệu khi di chuột, nhãn chặng "hỏng" trên đường tiến độ góp ý. Chỉ đổi màu chữ/nền của đúng ba luật, dùng token đậm sẵn có | 2026-09-11 | in_progress | `public/assets/css/style.css` — CHỈ ba luật `.tlq-nut-chinh`, `.tl-xem-dong:hover`, `.gy-td-chang.hong .gy-td-ten`. Không đụng JS, không đụng `vanphong.css`/`src/vp-*.js` |
