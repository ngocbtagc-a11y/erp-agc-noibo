...
| **Feedback Request** | Góp ý & Cải tiến ERP — vấn đề thực tế nhân viên báo, KHÔNG phải Task được giao (vòng đời triage→build→UAT khác hẳn) | Người gửi (tự); Admin triage | ERP | `gop_y`, `gop_y_lich_su` | `id` | — | Toàn công ty (Domain riêng, mở cho mọi vai trò) | KHÔNG | — | `trang_thai`, `loai` (chỉ Admin) | `tieu_de`, `boi_canh`, `vuong_o_dau`, `mong_muon`, `tan_suat`, `khu_vuc`, `dinh_kem` (chỉ người gửi, chỉ lúc tạo) |
...
| **R&D Project** *(bổ sung 06/09/2026 — xem `FEATURE-SPEC-RND-SANPHAM.md`)* | Dự án phát triển sản phẩm mới, chạy qua 12 giai đoạn cố định | P. Kinh Doanh-MKT (cùng Data Owner với Product/SKU) | ERP | `rnd_du_an` | `id` (`rnd_xxx`), `ma_rnd` (`RD0001`) | — | Kinh doanh | KHÔNG — **không chứa dữ liệu sản phẩm, chỉ trỏ `san_pham_id` sang bảng `san_pham` gốc (Rule 1)** | `id`, `ma_rnd` | `giai_doan` (chỉ đổi qua chuyển giai đoạn/quay lại, có cổng chặn + ghi sổ), `trang_thai` (qua đổi trạng thái, bắt buộc lý do khi dừng/huỷ) | `ten`, `nhom_hang`, `doi_tuong`, `kenh`, `uu_tien`, `phu_trach_id`, `han_ra_mat`, `gia_*_du_kien`, `y_tuong`, `ghi_chu` |
| **R&D Step** | 1 việc trong checklist của 1 giai đoạn, sinh sẵn lúc tạo dự án từ khuôn cứng `KHUON_QUY_TRINH` (`src/rnd.js`) | như R&D Project | ERP (khuôn quy trình nằm trong CODE, không phải dữ liệu người dùng tự sửa) | `rnd_buoc` | `id` (int) | — | Kinh doanh | KHÔNG | `giai_doan`, `thu_tu`, `ten`, `bat_buoc` (chốt lúc tạo dự án — dự án cũ giữ nguyên khi sửa khuôn, Rule 10) | — | `trang_thai` (`chua_lam`/`xong`/`khong_ap_dung`), `ket_qua` |
| **R&D History** | Sổ cái bất biến của dự án R&D | Hệ thống (tự ghi) | ERP | `rnd_lich_su` | `id` (int) | — | Kinh doanh | KHÔNG | toàn bộ — **chỉ INSERT, không UPDATE/DELETE** | — | — |
